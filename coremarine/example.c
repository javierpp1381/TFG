...
int main(void)
{
    ...
    while (fgets(line, sizeof(line), stdin) != NULL) {
        ...
        switch (minmea_sentence_id(line, false)) {
			...
			case MINMEA_SENTENCE_MWV: {
                struct minmea_sentence_MWV frame;
                if (minmea_parse_mwv(&frame, line)) {
                    printf(INDENT_SPACES "$xxMWV: %d %c %f %c %c\n",
                           frame.degrees,
                           frame.reference,
                           minmea_tofloat(&frame.windspeed),
                           frame.units,
                           frame.status);
                }
                else {
                    printf(INDENT_SPACES "$xxmwv sentence is not parsed\n");
                }
            } break;
