...
No solamente se mostrará a continuación el código modificado sino que se indicará su integración en el software Vivado HLS, del cual se hace uso para la síntesis en hardware y su optimización.
...
Tras comprobar que la FPGA tiene un uso correcto de sus recursos se puede exportar el Register Transfer Level (RTL) y generar la IP mediante la opción ``export RTL'' especificando que sea descrita en VHDL. Este bloque IP se utilizará en Vivado para configurar la FPGA para la aplicación del presente trabajo.
