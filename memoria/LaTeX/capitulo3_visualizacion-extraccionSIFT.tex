...
En primer lugar, se cargan las librerías de $iostream$ para las funcionalidades básicas de C++ y determinados módulos de la librería PCL; $IO$ para la lectura de nubes, $cloud_viewer$ del módulo $visualization$ para la visualización de nubes y $parse$ del módulo $console$ para poder evaluar los argumentos introducidos por la consola de comandos.
Además, se crea la instancia $sift\_ncloud$ de la clase $PointCloud$, es decir, una nube de puntos donde almacenar y modificar (resaltar puntos) la nube que contiene keypoints.
...
Por otra parte, si se introducen dos nubes de puntos, la original y la que contiene sus puntos SIFT, se ejecuta $viewerOneOffSIFT$ que hace lo mismo que la función anterior con el añadido de modificar la nube $sift\_ncloud$ para añadir esferas en cada uno de sus puntos y distinguirlos de los de la nube original.
Además, se configura el color de fondo del visualizador aportando pesos a los valores máximos de color rojo, verde y azul que se pueden tener; 1.0 para el color rojo y 0.5 para los colores verde y azul. 
...
Ahora se crea la instancia $cloud$ de la clase $PointCloud$ donde guardar la nube original, se muestra por pantalla el número de puntos contenidos en la misma y se crea la instancia del visualizador llamada $viewer$
...
Si se introducen dos nubes se carga la segunda en unanube llamada $sift\_cloud$ asigna la función $viewerOneOffSIFT$ al visualizador y se muestran las dos nubes en el mismo.
...
El nombre del ejecutable para este programa es $sift\_keypoints$ al cual se llama con dos tipos de argumentos; parámetros y nube de puntos. Los primeros son opcionales ya que tienen valores por defecto pero el segundo es obligatorio.\\
El esquema de llamada al ejecutable es el siguiente:
...
A continuación se crean e inicializan el conjunto de variables globales necesarias cuyo significado se explicará según se requiera su uso a lo largo del programa. Como excepción se tienen las variables $begin$, $end$ y $elapsed\_sec$ que sirven para medir tiempos de ejecución por lo que entrarán en juego en el siguiente capítulo.
...
Considerando una nube de puntos en dos dimensiones como una imagen, para construir un espacio escalado en primer lugar se toma la nube de puntos original con la función que la describe: 
$$f(x,y)$$ 
...
El proceso de creación del espacio escalado se puede visualizar en el ejemplo de la figura \ref{fig:filtro_gauss}  
...
El propósito de este conjunto de operaciones es el de simular diferentes escalas de observación (movimiento vertical en la figura \ref{fig:filtro_gauss}) así como la apreciación de diferentes niveles de detalle en cada escala (movimiento horizontal en la figura \ref{fig:filtro_gauss})
...
Se va a tomar como ejemplo una nibe de puntos llamada $five\_people.pcd$ y que representa cinco personas en un entorno cerrado.

Para visualizar esta nube se ejecuta el siguiente comando pudiendo ver el resultado en la figura \ref{fig:ejemplo_five_people}
$$./visualization\;\;five\_people.pcd$$



\begin{figure}[!htb]
\minipage{0.48\textwidth}
  \includegraphics[width=\linewidth]{ejemplo_five_people}
  \caption{Visualización de la nube $five\_people.pcd$}\label{fig:ejemplo_five_people}
\endminipage\hfill
\minipage{0.48\textwidth}
  \includegraphics[width=\linewidth]{ejemplo_five_people_sift}
  \caption{Visualización de la nube $five\_people.pcd$ junto a sus puntos SIFT representados por esferas blancas}\label{fig:ejemplo_five_people_sift}
\endminipage\hfill
...
ahora se quieren extraer los puntos SIFT de esta nube por lo que se ejecuta lo siguiente:
$$./sift\_keypoints\;\;five\_people.pcd$$

Los parámetros quedan por defecto y el resultado de la operación se muestra por pantalla tal y como se ve en la figura :



Por último, se visualizan en conjunto la nube original y sus puntos SIFT:
$$./visualization\;\;five\_people.pcd\;\;sift\_keypoints.pcd$$
El resultado final puede verse en la figura \ref{fig:ejemplo_five_people_sift}
